...
use ollama_rs::{generation::{chat::{ChatMessage}, completion::{request::GenerationRequest}}, Ollama};
...
impl AIRepository for GenAIRepository {
    async fn get_ai_models(&self) -> Result<Vec<ModelResponse>, String> {
        ...
        for model in local_models {
            ...
            let handle = tokio::spawn(async move {
                let model_info = ollama_clone.show_model_info(model_name.clone()).await
    ...
    async fn chat_with_ai(
        ...
        let ollama = Ollama::default();
        let full_prompt = prompt.iter().map(|m| m.content.clone()).collect::<Vec<String>>().join("\n");
        ...
        tokio::spawn(async move {
            ...
            match res {
                Ok(mut stream) => {
                    let _ = stream_response_to_frontend(window, &mut stream, new_token_for_task).await;
                }
                Err(e) => {
