...
/// a problem ocurred.
#[doc(alias = "get_notes")]
pub async fn get_notes_use_case<T: NoteRepository>(repo: &T, space_name: &str) -> Result<Vec<Note>, NoteError> {
...
pub async fn get_note_content_use_case<T: NoteRepository>(repo: &T, space_name: &str, note_name: &str) -> Result<Note, NoteError> {
    repo.get_note_content(space_name, note_name).await
}