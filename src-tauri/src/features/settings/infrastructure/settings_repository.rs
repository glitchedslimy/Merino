...
use crate::{features::{settings::domain::repository::SettingsRepository}, shared::repositories::filesystem_repository::FileSystemRepository};
...
impl SettingsRepository for FileSystemSettingsRepository {
    async fn create_settings(&self) -> Result<(), String> {
        ...
        if !settings_path.exists() {
            ...
            fs::write(&settings_path, &default_settings).await.map_err(|e| format!("Failed to create settings file: {}", e))?;
        }
        Ok(())
    }
   ...
   async fn get_settings(&self) -> Result<String, String> {
    ...
    async fn update_settings(&self, new_setting: String) -> Result<(), String> {
        ...
        let mut current_settings: Value = serde_json::from_str(&current_settings_str).unwrap_or_else(|_| json!({}));

        let new_setting_value: Value = serde_json::from_str(&new_setting).map_err(|e| format!("Failed to parse new setting JSON: {}", e))?;
        ...
        let merged_settings_str = serde_json::to_string(&current_settings).map_err(|e| format!("Failed to serialize merged settings: {}", e))?;

        fs::write(&settings_path, &merged_settings_str).await.map_err(|e| format!("Failed to write new settings: {}", e))?;
