...
use crate::{
    features::theming::domain::{repository::ThemingRepository, theme::Theme},
...
impl ThemingRepository for FileSystemThemingRepository {
    async fn get_themes(&self) -> Result<Vec<Theme>, String> {
        ...
        if themes_path.exists() && themes_path.is_dir() {
            let mut entries = read_dir(themes_path)
                .await
    ...
    async fn create_themes_path(&self) -> Result<(), String> {
        ...
        if !theme_path.exists() {
            fs::create_dir_all(&theme_path)
                .await
                .map_err(|e| format!("Couldn't create the theming directory: {}", e))?;
        }
        Ok(())
    }
}
