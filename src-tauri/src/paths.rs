...
pub async fn ensure_app_directories_exists(app_handle: &AppHandle) -> Result<(), String> {
    let app_data_root_dir = app_handle.path().app_data_dir().or_else(|_| Err("Could not determine application data directory for creation"))?;

    if !app_data_root_dir.exists() {
        return fs::create_dir_all(&app_data_root_dir).await.map_err(|e| format!("Failed to create app data root directory. {}", e));
    } 

    let notes_base_path = app_data_root_dir.join(BASE_DIR_NAME);

    if !notes_base_path.exists() {
        return fs::create_dir_all(&notes_base_path).await.map_err(|e| format!("Failed to create notes base directory. {}", e));
    } 
